...
class Node_Embedding(nn.Module):
    def __init__(self, input_size, output_size, raw_features):
        super().__init__()
    ...
    def forward(self, batch_nodes = []):
        if len(batch_nodes) == 0:
            batch_nodes = np.arange(len(self.raw_features))
        return self.layers(self.raw_features[batch_nodes, :])
...
class Classification(nn.Module):
    ...
    def predict(self, embeds):
        with torch.no_grad():
            ...
            train_nodes = torch.Tensor(np.arange(num)).type(torch.LongTensor)
            batches = math.ceil(len(train_nodes) / b_sz)
            
            for index in range(batches):
                nodes_batch = train_nodes[index*b_sz:(index+1)*b_sz]
                features = embeds[nodes_batch]
                
                outputs = self.layer(features)
        ...
        if self.agg_func == 'MEAN':
            ...
            for feat in [embed_matrix[x.squeeze()] for x in indexs]:
                ...
                if len(feat.size()) == 1:
                    ...
                    #print('[FEAT]', feat.shape)
                    aggregate_feats.append(torch.zeros(1, feat.size(1)).to(self.device))
                else:
                    aggregate_feats.append(torch.max(feat,0)[0].view(1, -1))  
            aggregate_feats = torch.cat(aggregate_feats, 0)

        #print('Aggregation size = ', aggregate_feats.size())        
...
#import torch.nn as nn
class GraphSage_XEmbed(nn.Module):
    """docstring for GraphSage"""
    def __init__(self, num_layers, input_size, out_size, raw_features, adj_lists, device, gcn=False, agg_func='MEAN'):
        #super(GraphSage, self).__init__()
        super().__init__()
        ...
        self.node_embedding = Node_Embedding(input_size, input_size, raw_features)
    ...
    def reset_features(self, features):
        self.node_embedding.raw_features = features
    
    def get_features(self):
        return self.node_embedding.raw_features
    
    def get_datasetSize(self):
        return len(self.node_embedding.raw_features)
    ...
    def forward(self, nodes_batch):
        ...
        pre_hidden_embs = self.node_embedding()
